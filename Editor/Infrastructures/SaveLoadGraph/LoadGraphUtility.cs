...
namespace SimpleDialogueSystem.Editors
{
    public static class LoadGraphUtility
    {
        ...
        private static Dictionary<DialogueNodeStaticData, SimpleNode> _pair;
        ...
        {
            _pair = new Dictionary<DialogueNodeStaticData, SimpleNode>();
            _dialogueNodes = new();
        ...
        private static void RestoreConnections()
        {
            foreach (var node in _pair.Keys)
                ...
                {
                    SimpleNode nextNode = (SimpleNode)FindEditorNode(nextNodeID);
            ...
            {
                SimpleNode nextNode = (SimpleNode)FindEditorNode(_dialogueStaticData.StartNodeID);
        ...
        private static void CreateEditorNodes()
        {
            if (_dialogueStaticData != null)
            {
                ...
                foreach (var node in _dialogueStaticData.NodeDatas)
                {
                    ...
                    SimpleNode editorNode = _graphView.CreateNode(node.Position.ToVector2(), events, node.ID);
        ...
        private static void CreateNoteNodes()
        {
            if (_dialogueStaticData != null)
            {
                ...
                {
                    _graphView.CreateStickyNote(node.Position, node.Text);
                }
            }
        }
